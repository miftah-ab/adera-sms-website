...
export default function ProductProof() {
  ...
  return (
    <section className="relative border-y border-white/[0.06] bg-[#07130b] py-20 lg:py-28 overflow-hidden">
      {/* Background glow accent */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[350px] w-[700px] rounded-full bg-emerald-500/[0.06] blur-[100px] pointer-events-none" />
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 lg:px-8">
          ...
          {steps.map((step, idx) => (
            <div
              ...
              className={`group relative flex flex-col justify-between rounded-2xl p-6 transition-all duration-300 cursor-pointer border ${
                activeStep === step.id
                  ? "bg-[#0d2618] border-emerald-500/50 shadow-[0_10px_30px_-10px_rgba(16,185,129,0.3)]"
            ...
            >
              ...
              <div className="mt-6 pt-4 border-t border-white/[0.06]">
                <div className="rounded-lg bg-black/40 px-3 py-2 text-xs font-mono text-zinc-300 border border-white/[0.04] truncate">
                  {step.snippet}
                </div>
              </div>
            </div>
        ...
        {/* Narrative Flow Bar */}
        <div className="mt-10 flex flex-wrap items-center justify-center gap-2 sm:gap-4 text-xs sm:text-sm font-medium text-zinc-400">
          <span className="text-zinc-200">CALL</span>
          <span className="text-emerald-500">→</span>
          <span className="text-zinc-200">MISSED CALL</span>
          <span className="text-emerald-500">→</span>
          <span className="text-emerald-400 font-bold">ADERA</span>
          <span className="text-emerald-500">→</span>
          <span className="text-zinc-200">AUTOMATIC TEXT</span>
          <span className="text-emerald-500">→</span>
          <span className="text-emerald-300 font-semibold">CALLER REASSURED</span>
        </div>
